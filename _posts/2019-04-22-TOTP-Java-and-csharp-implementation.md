...
2. **扫码登录**，TOTP算法没有解决的一个问题就是客户端和服务端之间的共享密钥问题，我们可以通过把共享密钥也做成一个动态值，然后把动态密钥做成二维码，客户端通过扫码得到动态密钥，最后在客户端计算TOTP值发送给服务器。微信扫码、支付宝扫码就是这个算法的应用，也就是说，一般会有一个手机端的App；
3. **硬件TOKEN**，国内部分公司的工卡上携带一个硬件TOKEN，比如腾讯的TOKEN估计就是这个算法的应用；
